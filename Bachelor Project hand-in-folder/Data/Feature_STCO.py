...
def create_feature_for_entire_df(fx_pair, year, time_frame):
    ...
    df[feature_name] = (df['Close'] - df['Close'].rolling(window=time_frame , min_periods=0).min() ).div(
                    df['Close'].rolling(window=time_frame , min_periods=0).max() - 
                    df['Close'].rolling(window=time_frame , min_periods=0).min())
